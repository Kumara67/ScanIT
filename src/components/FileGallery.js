...
export default FileGallery = (props) => {
  const [imageArray, setImageArray] = useState([]);
  const [selectedItems, setSelectedItems] = useState([]);
  const [rerender, forceRerender] = useState('someKey');
  let folderPath = props.route.params.folderPath;
  let folderName = folderPath.slice(folderPath.lastIndexOf('/')+1,  folderPath.length)
